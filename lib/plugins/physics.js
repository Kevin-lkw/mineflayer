...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // this part (sigal) is from mineflayer-pathfinder
  let Goal = null
  ...
  // actually send is (a_t,s_{t+1}). s is simulated under at
  function sendPacketPosition (position, onGround, action) {
    // sends data, no logic
    ...
    // console.log("[physics] sending",packet)
    bot.emit('sentPosAction', packet)
  }

  function sendPacketLook (yaw, pitch, onGround, action) {
    // sends data, no logic
    const oldPos = new Vec3(lastSent.x, lastSent.y, lastSent.z)
    lastSent.yaw = yaw
  ...
  function sendPacketPositionAndLook (position, yaw, pitch, onGround, action) {
    // sends data, no logic
    const oldPos = new Vec3(lastSent.x, lastSent.y, lastSent.z)
    lastSent.x = position.x
    lastSent.y = position.y
  ...
  function updatePosition (now) {
    ...
    action = {
      ...
      sneak: controlState.sneak,
      camera: [math.clamp(-maxDeltaYaw, dYaw, maxDeltaYaw), math.clamp(-maxDeltaPitch, dPitch, maxDeltaPitch)],
    }
    // console.log("[physics] action",action)
    if (positionUpdated && lookUpdated) {
      sendPacketPositionAndLook(position, yaw, pitch, onGround, action)
      lastSent.time = now // only reset if positionUpdated is true
    } else if (positionUpdated) {
      sendPacketPosition(position, onGround, action)
      lastSent.time = now // only reset if positionUpdated is true
    } else if (lookUpdated) {
      sendPacketLook(yaw, pitch, onGround, action)
  ...
  bot.on('sentPosAction', () => {
    if (!lookingTask.done && Math.abs(deltaYaw(bot.entity.yaw, lastSentYaw)) < 0.01) {
      // console.log("BBBB")
      lookingTask.finish()
      bot.emit('lookingdone')
    }
  })
  /*
    return 0 if lookat goal need to be reset
    return 1 if lookat goal is correct and done
    return 2 if lookat goal is correct and unfinish
  */
  bot.checklook = (yaw, pitch) => {
    // new yaw and pitch
    if (Math.abs(deltaYaw(bot.entity.yaw, yaw)) > 0.01) {
      return 0
    }
    if (Math.abs(bot.entity.pitch - pitch) > 0.01) {
      return 0
    }
    if (lookingTask.done === true){
      return 1
    }
    return 2
  }
  ...
  bot.look = async (yaw, pitch, force) => {
    ...
    if (Math.abs(bot.entity.yaw) > PI) {
      bot.entity.yaw -= Math.sign(bot.entity.yaw) * PI_2
    }
  ...
  bot._client.on('position', (packet) => {
    ...
    sendPacketPositionAndLook(pos, newYaw, newPitch, bot.entity.onGround, empty_action)
